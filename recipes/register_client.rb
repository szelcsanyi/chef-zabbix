  ...
  #Chef::Log.info("[Zabbix] Api version: #{version}")
    ...
    if zbxnodeid.nil?
        ...
        if node["public_ipaddress"].empty?
            interfaces = [{ :type => 1, :main => 1, :ip => node["ipaddress"], :dns => node["fqdn"], :port => 10050, :useip => 1 }]
        else
            interfaces = [{ :type => 1, :main => 1, :ip => node["public_ipaddress"], :dns => node["fqdn"], :port => 10050, :useip => 1 }]
        end
        interfaces << { :type => 1, :main => 0, :ip => node["public_ip6address"], :dns => node["fqdn"], :port => 10050, :useip => 1 } unless node["public_ip6address"].empty?
        zbxnodeid = zbx.query(
            :method => "host.create",
            :params => {
                :available => 1,
        ...
        Chef::Log.info("[Zabbix] Host #{node.fqdn} is already registered in Zabbix (#{zbxnodeid})!")
    end
